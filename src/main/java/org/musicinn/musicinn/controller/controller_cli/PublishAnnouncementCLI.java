...
public class PublishAnnouncementCLI {
    ...
    public void run() {
        ...
        try {
            ...
            bean.setDoesUnreleased(unreleased.equals("1") ? true : (unreleased.equals("2") ? false : null));
    ...
    private void displayVenueSummary(VenueBean venue) {
        LOGGER.info("Locale: " + venue.getName());
        LOGGER.info("Indirizzo: " + venue.getAddress() + ", " + venue.getCity());
        LOGGER.info("Rider Tecnico attuale:\n" + TechnicalRiderFormatter.format(venue.getRider(), Session.UserRole.MANAGER));
        LOGGER.info("------------------------------------------------");
    }
